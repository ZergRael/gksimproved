...
var snatched_authorized = ["/m/peers/snatched"];
if($.inArray(url.path[0], snatched_authorized) != -1) {
	loadModule = true;
}

if(loadModule) {
	var module = {name:"snatched"};
	...
	switch(url.path[0]) {
		case "/m/peers/snatched":
			module.buttons = '#contenu > a:first';
	...
	var jOnScroll = function() {
		if(endlessScrolling) {
			...
			if((document.body.scrollTop + window.innerHeight > document.body.scrollHeight - scrollOffset) && !loadingPage) {
				...
				$(module.loading).before('<p class="pager_align page_loading"><img src="' + chrome.extension.getURL("images/loading.gif") + '" /><br />Réticulation des méta-données de la page suivante</p>');
	...
	var filteringDeleted = opt.get(module.name, "filtering_deleted");
	...
	var grabAllPages = function() {
		...
		$(module.loading).before('<p class="pager_align page_loading"><img src="' + chrome.extension.getURL("images/loading.gif") + '" /><br />Hachage positronique de l\'ensemble des pages</p>');
	...
	$("#filter_deleted").change(function() {
		...
		opt.set(module.name, "filtering_deleted", filteringDeleted);
		if(filteringDeleted) {
