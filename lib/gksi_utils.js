...
var utils = {
	...
	storage: {
		...
		legacy: {
			...
			data_rm: function(module) {
				localStorage.removeItem("data_" + module);
			}
		}
	},
	...
	getAura: function() {
		...
		return Number($("#userlink li:not(#user_buffer):nth(3)").text().match(/[\d\.]+/)[0]);
	},
	...
	getSiteRelativeDate: function() {
		...
		var time = d.getTime();
		var isDST = (dstStart.getTime() > time) && (dstEnd.getTime() < time);
